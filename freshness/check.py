"""Weekly freshness check for the sources behind policy-catalogue/.
...
UA = "oss-ai-contribution-policy freshness check (+https://github.com/ecogetaway/oss-ai-contribution-policy)"
REPO = os.environ.get("GITHUB_REPOSITORY", "ecogetaway/oss-ai-contribution-policy")
TOKEN = os.environ.get("GITHUB_TOKEN", "")
LABEL = "source-changed"
TITLE = "Catalogue sources changed: review needed"
...
def main():
    ...
    if changed:
        parts.append("## Changed since last verification\n\nReview each change against the catalogue entry and the comparison page, then update `verified` in `freshness/watchlist.yml`.\n\n" + "\n\n".join(changed))
    if unreachable:
    ...
    if args.dry_run or not needs_issue:
        return 0
    try:
        gh(f"/repos/{REPO}/labels", "POST", {"name": LABEL, "color": "d4c5f9", "description": "A catalogue source changed; review needed"})
